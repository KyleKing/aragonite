...
func SearchPRsInRepo(ctx context.Context, repoPath, remoteID, query string) ([]forge.PullRequest, error) {
	...
	if cached, ok := PRSearchCache.Get(key, vcs.Stamp(repoPath)); ok {
		return cached, nil
	}
	...
	PRSearchCache.Set(key, vcs.Stamp(repoPath), found)
...
func SearchPRsEverywhere(ctx context.Context, repoPath, query string) ([]forge.PullRequest, error) {
	...
	if cached, ok := PRSearchCache.Get(key, cache.NoStamp); ok {
		return cached, nil
	}
	...
	PRSearchCache.Set(key, cache.NoStamp, found)
