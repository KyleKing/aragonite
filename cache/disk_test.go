...
func samplePRs() []prInfo {
	return []prInfo{
		{Number: 7, Title: "Cache pull requests on disk", State: "OPEN", HeadRef: "cache", Checks: checksStatus{
			Total: 2, Passing: 2,
		}},
...
func TestPersistedSurvivesAProcessBoundary(t *testing.T) {
	...
	cache.PersistUsing(cache.NewDiskCache(dir), cache.NewTTLCache[[]prInfo](time.Hour),
		upstream, listKey, stamp, samplePRs())

	got, ok := cache.PersistedUsing(cache.NewDiskCache(dir), cache.NewTTLCache[[]prInfo](time.Hour),
	...
	if _, ok := cache.PersistedUsing(cache.NewDiskCache(dir), cache.NewTTLCache[[]prInfo](time.Hour),
...
func TestUnreadableFilesAreAMiss(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cache.PersistUsing(store, cache.NewTTLCache[[]prInfo](time.Hour),
...
func TestNoBodyReachesDisk(t *testing.T) {
	...
	cache.PersistUsing(store, cache.NewTTLCache[[]prInfo](time.Hour), upstream, listKey, cache.NoStamp, prs)
	...
	for _, persisted := range []any{[]prInfo{}, map[string]string{}} {
...
func TestConcurrentWritersLeaveNoHalfWrittenFile(t *testing.T) {
	...
	for writer := range 8 {
		...
		go func() {
			...
			store := cache.NewDiskCache(dir)
			memory := cache.NewTTLCache[[]prInfo](time.Hour)
			prs := []prInfo{{Number: writer, Title: strings.Repeat("t", 512)}}

			for round := range 20 {
				cache.PersistUsing(store, memory, upstream, "prs:"+strconv.Itoa(round), cache.NoStamp, prs)

				got, ok := cache.PersistedUsing(store, cache.NewTTLCache[[]prInfo](time.Hour),
...
func TestNoStoreWritesNothing(t *testing.T) {
	...
	if _, ok := cache.PersistedUsing(nil, cache.NewTTLCache[[]prInfo](time.Hour),
...
func TestAnEmptyUpstreamStaysInMemory(t *testing.T) {
	...
	cache.PersistUsing(store, cache.NewTTLCache[[]prInfo](time.Hour), "", listKey, cache.NoStamp, samplePRs())
...
func TestEvictionDropsTheLeastRecentlyUsed(t *testing.T) {
	...
	cache.PersistUsing(store, cache.NewTTLCache[[]prInfo](time.Hour), kept, listKey, cache.NoStamp, samplePRs())
...
func TestAMovedStampMissesOnDisk(t *testing.T) {
	...
	cache.PersistUsing(store, cache.NewTTLCache[[]prInfo](time.Hour), upstream, listKey, written, samplePRs())

	moved := cache.Stamp{Scope: "/repo", Fingerprint: "head-b"}
	if _, ok := cache.PersistedUsing(store, cache.NewTTLCache[[]prInfo](time.Hour),
...
func TestExpiredEntriesAreNeitherServedNorKept(t *testing.T) {
	...
	cache.PersistUsing(store, cache.NewTTLCacheWithClock[[]prInfo](5*time.Minute, clock.now),
		upstream, listKey, cache.NoStamp, samplePRs())

	clock.advance(time.Hour)

	if _, ok := cache.PersistedUsing(store, cache.NewTTLCacheWithClock[[]prInfo](5*time.Minute, clock.now),
		upstream, listKey, cache.NoStamp); ok {
