// Package ghcassette records and replays gh subprocess calls. A tool that
// reads GitHub by shelling out to gh has its seam at PATH rather than inside
// an HTTP transport, so the cassette puts a stand-in gh ahead of the real one
// and either records what the real binary did or answers from a file. A test
// then replays the bytes GitHub actually sent, in the shape gh actually prints
// them.
//
...
// tests and finds nothing in production. gh run view --log prefixes every line
// with "job\tstep\ttimestamp message", stores GitHub's coloring as caret
// notation rather than escape bytes, and opens with a byte order mark; a
// fixture that misses any of that is a test of the author's imagination.
//
// transport is the counterpart for a client built on http.RoundTripper.
//
...
type Interaction struct {
	...
	// Stdin is the body gh was told to read from it, recorded for the reader
	// rather than matched on: the args already identify the call.
