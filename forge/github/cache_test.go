...
func useTempDiskCache(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		github.PRCache.Clear()
		github.DefaultBranchCICache.Clear()
	})
}
...
func TestGitHubValuesSurviveAColdMemoryCache(t *testing.T) {
	...
	cache.Persist(github.PRCache, testRemoteID, prKey, cache.NoStamp, pr)
	...
	cache.Persist(github.DefaultBranchCICache, testRemoteID, ciKey, cache.NoStamp, ci)
