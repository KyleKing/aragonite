...
func GetWorkflowRunsForCommit(
	...
	if cached, ok := WorkflowCache.Get(cacheKey, vcs.Stamp(repoPath)); ok {
		return cached, nil
	}
	...
	WorkflowCache.Set(cacheKey, vcs.Stamp(repoPath), summary)
...
func CachedDefaultBranchCI(ctx context.Context, repoPath, remoteID string) (*forge.DefaultBranchCI, bool) {
	...
	return cache.Persisted(DefaultBranchCICache, remoteID,
...
func GetDefaultBranchCI(ctx context.Context, repoPath, remoteID string) (*forge.DefaultBranchCI, error) {
	...
	if cached, ok := cache.Persisted(DefaultBranchCICache, remoteID, cacheKey, cache.NoStamp); ok {
		return cached, nil
	}
	...
	cache.Persist(DefaultBranchCICache, remoteID, cacheKey, cache.NoStamp, ci)
